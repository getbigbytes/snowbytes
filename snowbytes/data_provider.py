...
#             # eg. SNOWBYTES_DEV.PUBLIC."FETCH_DATABASE(NAME VARCHAR):OBJECT"
#             # The correct FQN is SNOWBYTES_DEV.PUBLIC."FETCH_DATABASE"(VARCHAR)
...
def fetch_role_privileges(
    ...
    for role in roles:
        ...
        for grant in grants:
            ...
            # If snowbytes isnt aware of the privilege, ignore it
            except ValueError:
                continue
...
def fetch_resource_tags(session: SnowflakeConnection, resource_type: ResourceType, fqn: FQN):
    ...
    +----------------------+------------+-------------+-----------+--------+----------------------+---------------+-------------+--------+-------------+
    | SNOWBYTES                | SOMESCH    | TASTY_TREAT | muffin    | SCHEMA | TEST_DB_RUN_13287C56 |               | SOMESCH     | SCHEMA |             |
...
def list_grants(session: SnowflakeConnection) -> list[FQN]:
    ...
    for role in roles:
        ...
        for data in grant_data:
            ...
            # Snowbytes Grants don't support OWNERSHIP privilege
