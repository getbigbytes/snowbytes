...
def _resources_from_users_config(users_config: list) -> list:
    ...
    for user in users_config:
        if isinstance(user, dict):
            ...
            snowbytes_user = User(**user)
            resources.append(snowbytes_user)
            for role in roles:
                resources.append(
                    RoleGrant(
                        role=role,
                        to_user=snowbytes_user,
                    )
                )
...
def crawl(path: str):
    # Load .snowbytesignore patterns if the file exists
    gitignore_path = os.path.join(path, ".snowbytesignore")
    ...
    for root, _, files in os.walk(path):
        for file in files:
            if file.endswith(".yaml") or file.endswith(".yml"):
                ...
                # Get path relative to the base path for snowbytesignore matching
...
def collect_vars_from_environment() -> dict:
    ...
    for key, value in os.environ.items():
        if key.startswith("SNOWBYTES_VAR_"):
