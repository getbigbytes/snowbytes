...
from snowbytes.enums import ResourceType, WarehouseSize
from snowbytes.resource_name import ResourceName
from snowbytes.resource_tags import ResourceTags
from snowbytes.resources.resource import ResourcePointer
from snowbytes.resources.user import UserType
from snowbytes.resources.view import ViewColumn
...
def test_resource_with_named_nested_dependency():
    ...
    NetworkRules. Snowbytes tries to support as many common-sense compositions of this input
    as possible.
    ...
    Unfortunately for snowbytes, we're not a database. In Snowflake, name resolution always happens
    ...
    Snowbytes serializes resources far before a session is initiated, so we don't have that luxury.

    Why can't we just keep the string? Snowbytes automatically managed implied references. Snowbytes
