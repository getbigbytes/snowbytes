...
from snowbytes.client import reset_cache
from snowbytes.enums import BlueprintScope, ResourceType
from snowbytes.exceptions import NotADAGException
from snowbytes.gitops import collect_blueprint_config
from snowbytes.resources.database import public_schema_urn
...
def test_blueprint_quoted_references(cursor):
    session = cursor.connection
    try:
        cursor.execute('CREATE USER IF NOT EXISTS "developers@getbigbytes.com"')
        cursor.execute('GRANT ROLE STATIC_ROLE TO USER "developers@getbigbytes.com"')

        blueprint = Blueprint(
            name="test_quoted_references",
            resources=[res.RoleGrant(role="STATIC_ROLE", to_user="developers@getbigbytes.com")],
        )
        plan = blueprint.plan(session)
        assert len(plan) == 0
    finally:
        cursor.execute('DROP USER IF EXISTS "developers@getbigbytes.com"')
...
def test_blueprint_grant_with_lowercase_priv_drift(cursor, suffix, marked_for_cleanup):
    ...
    def _blueprint():
        blueprint = Blueprint()
        role = res.Role(name=f"SNOWBYTES_TEST_ROLE_{suffix}")
        warehouse = res.Warehouse(
            name=f"SNOWBYTES_TEST_WAREHOUSE_{suffix}",
...
def test_blueprint_share_custom_owner(cursor, suffix):
    ...
    share = res.Share(name=share_name, owner="SNOWBYTES_SHARE_ADMIN")
