...
from snowbytes.privs import all_privs_for_resource_type
from snowbytes.identifiers import URN
...
def test_grant_refs():
    grant = res.Grant.from_sql("GRANT READ ON IMAGE REPOSITORY some_repo TO ROLE snowbytes_app_admin")
    repo = res.ImageRepository(
        name="snowbytes_app_image_repo",
        owner="snowbytes_app_admin",
        database="snowbytes_app_db",
        schema="snowbytes_app",
    )
    assert ResourceName(str(repo.fqn)) == ResourceName("snowbytes_app_db.snowbytes_app.snowbytes_app_image_repo")
