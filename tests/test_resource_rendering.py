import snowbytes.resources as res
...
def test_resource_pointer_rendering():
    ...
    network_rule = res.NetworkRule(
        name="SNOWBYTES_TEST_NETWORK_RULE",
        type="IPV4",
    ...
    network_policy = res.NetworkPolicy(
        name="SNOWBYTES_TEST_NETWORK_POLICY",
    ...
    assert (
        rendered
        == "CREATE NETWORK POLICY SNOWBYTES_TEST_NETWORK_POLICY ALLOWED_NETWORK_RULE_LIST = (DB.SCH.SNOWBYTES_TEST_NETWORK_RULE)"
    )
